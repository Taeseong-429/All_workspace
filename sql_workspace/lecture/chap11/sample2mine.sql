...
--          a. Database-level Privileges --> 데이터 베이스 수준의 권한. 
...
-- Basic syntax:
--  
--  GRANT 시스템권한1 [, 시스템권한2, ...] -- 부여
--  TO { 사용자계정 | role | PUBLIC } --> 누구에게/ 역할/ PUBLIC:현재 존재하는 모든 일반계정한테 주겠다
--  [WIDTH ADMIN OPTION] ; --> 부여받은 계정이 다른 계정에게 또 부여할 권한을 주겠다.(즉, 권한을 위임) 라는 의미. 생략가능한 부분..
--
--   * 시스템권한[1~n]: 부여할 시스템 권한 항목 나열

--   * role(역할): 한 개 이상의 권한들의 집합(****)
-- --    권한의 부여/취소 작업을 용이하게 하기 위해 사용 --> 취소 : revoke
...
-- 할당(부여)
CREATE USER user01
IDENTIFIED BY oracle; -- 계정만들기. 들어가기: cmd에서 sqlplus user1

GRANT 
    CREATE SESSION,
    CREATE TABLE
TO
    user01; ---이렇게 개별적으로 시스템 설정 하는게 아니고 아래 있는 혹은 노션에 필기한 대로 지정해야함.

-- CREATE SESSION 시스템 권한 테스트
-- 접속설정을 통해 확인 (VSC or Oracle SQL*Developer or Oracle SQL*PLUS)
----> developer에서 계정 찾아 들어가서 사용자 편집. USERES

GRANT CONNECT, RESOURCE TO USER01; ——> 이렇게 권한 부여해야함. 

CREATE USER newuser iDENTIFIED BY newpassword 

DEFAULT TABLESPACE users라고 하는 테이블 스페이스

TEMPORARY TABLESPQCE temp; 이렇게 해야 가능.
...
-- ------------------------------------------------------
-- 2-2. SESSION_PRIVS 데이터 사전 조회  --> 접속한 상태에서 확인 가능한 것.
-- ------------------------------------------------------
...
SELECT *
FROM session_privs; ---> 지금 접속한 계정의 상태에서 사용 가능한 권한 확인 하는 방법!
...
WHERE deptno = 50;      -- XX: DELETE 객체권한은 부여받지 못했음
