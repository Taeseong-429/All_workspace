...
-- (1) PK 제약조건 삭제 (2가지 방법)
-- ------------------------------------------------------
-- 1st. method --> 이걸 권하지는 않음. 아래처럼 이름을 가지고 하는게 좋음.
ALTER TABLE dept03
DROP PRIMARY KEY;

-- 2nd. method
ALTER TABLE dept03
DROP CONSTRAINT dept03_deptno_pk; --> 이 방법으로 제약조건 삭제하자 !
...
ALTER TABLE dept03
DROP CONSTRAINT SYS_C007020; -- 무작위로 설정된 제약조건 삭제
...
ALTER TABLE dept03
ADD PRIMARY KEY(deptno); --> 제약조건 con~ 이거 안쓰고 입력하면 오라클이 이름 대신 지어줌.
