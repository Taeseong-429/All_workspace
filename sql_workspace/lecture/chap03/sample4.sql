...
SELECT
    ...
    to_char(1234, '$99999') AS "$99999",        -- $: 달러 기호를 앞에 표현 -- > 이런거 공백 있어야 하는데, 개발도구에 따라 달라짐. 여기선 왼쪽정렬이라 공백 없음.
    to_char(1234, '99999.99') AS "99999.99",    -- .: 소수점을 표시
    to_char(1234, '99,999') AS "99,999",        -- ,: 특정 위치에 , 표시
    to_char(1234, 'B9999.99') AS "B9999.99",    -- B: 공백을 0으로 표현 --> 공백이 만약 남는다면 0으로 만들어라.
...
SELECT 
    last_name,
    salary,
    to_char(salary, '$999,999') 달러, --> 세자리마다 표시해주고, 달라를 붙여주어라.
