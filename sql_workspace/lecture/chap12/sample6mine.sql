...
BEGIN       -- 실행부(Execution section) 시작
    ...
    UPDATE emp
    SET sal = sal * 1.1         -- 110% 인상 급여로 수정
    WHERE empno = p_empno;      -- 전달받은 사원번호로 사원급여 수정
...
CREATE OR REPLACE PROCEDURE format_phone (
    ...
    p_phone_no IN OUT VARCHAR2  --> IN OUT 알아두기. 여기서 IN으로 넣고, 아래 PRINT 이걸로 OUT 출력하는것같음.
)
IS 
-- 지역변수 선언부
    --  a. 반드시 변수의 길이를 지정해야 함
...
BEGIN 
    ...
    p_phone_no := '(' ||
                    substr(p_phone_no, 1, 3) || ')' || --> 3자리 끊어내고
                    substr(p_phone_no, 4, 4) || ')' || --> 4자리 
                    substr(p_phone_no,7);
END;

EXEC Format_phone('0100111122222'); -- > INOUT 매개변수는 우리가 전에 쓰던걸로 호출할 수 없음. 오류남.
...
SELECT DISTINCT name --> 내가 생성한 프로시저 이름을 들고있다.
FROM user_source;
