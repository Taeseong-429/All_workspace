...
--  자. 예외처리 기본문법:
--      Exception_handling_syntax.JPG --> 다중캐치는 WHEN이 여러개, 멀티캐치는 하나의 WHEN 에 OR을 써서 여러개 .
--
...
BEGIN
    SELECT * 
        INTO v_emp --> 하나의 레코드만 집어넣을 수 있는데, 조건을 맞춘게 한 개 이상이 나타날 수 있음.
    FROM emp
    WHERE deptno = 30;
...
BEGIN
    INSERT INTO dept
    VALUES (500, '인사', '서울'); --> 40이 PRIMARY KEY 일때, 만약 넣는데 이미 있는 값이라면? 걱정되면 예외 만들기.
    --> 500 일때 오류 발생하는 이유는 데이터 타입이 2자리라서.

    DBMS_OUTPUT.put_line('저장 성공');

    EXCEPTION
        WHEN DUP_VAL_ON_INDEX THEN
            DBMS_OUTPUT.put_line('중복 데이터 저장');

        -- WHEN OTHERS THEN
        --     DBMS_OUTPUT.put_line('중복 데이터 저장 위배 예외발생');

        WHEN OTHERS THEN
            DBMS_OUTPUT.put_line('예외발생');
            DBMS_OUTPUT.put_line('----------------------');

            v_code := SQLCODE; -- A Function to show Oracle Error Code
            v_errm := SQLERRM; -- A Function to show Oracle Errer Message

            DBMS_OUTPUT.put_line('SQLCODE:' || v_code);
            DBMS_OUTPUT.put_line('SQLERRM:' || v_errm);
...
-- ------------------------------------------------------
-- 예1: 조건에 일치하는 사원이 없는 경우(논리적 오류), 사용자 정의 예외를
--      사용하여, 처리하는 방법
-- ------------------------------------------------------
