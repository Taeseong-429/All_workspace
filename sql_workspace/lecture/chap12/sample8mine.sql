...
--     외부에서 참조가능--> 함수의 선언부만 있는 것을 함수 프로토타입 이라고 함./ 프로시저도 프로시저 헤더라고 부름.
-- (구현부는 빠져있는 것)
...
-- ------------------------------------------------------
-- 8-2. PL/SQL 패키지: 패키지 구현부 --> 패키지 바디라고 부름.
-- ------------------------------------------------------
...
-- ------------------------------------------------------
-- 예1: 구구단 프로시저와 문자열 소문자 변경 함수를 패키지로 생성
-- ------------------------------------------------------
...
IS
    ...
    function circleArea(p_radius number) return number; --> 패키지 만들 땐 대문자보단 소문자 씀.

    PI CONSTANT NUMBER := 3.14159;  --> 이렇게 선언부에서 선언하면 외부에서도 쓸수 있음.
END mypackage;
...
IS
   -- 특정 단수의 구구단 출력 프로시저 구현
    PROCEDURE gugudan (dansu NUMBER) --> CREATE 안쓰고 바로 실행부 입력
    IS
    BEGIN
        FOR i IN 1..9 LOOP
    ...
    as
    begin
        return PI * p_radius * p_radius; --> PI 를 위에서 지정해줬음.
    end; 
END mypackage;
