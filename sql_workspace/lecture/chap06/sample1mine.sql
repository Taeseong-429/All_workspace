...
--          - 사용가능 연산자: IN, ANY, ALL, EXIST 등 --> ANY: 하나라도 참이면 참이되는. ALL: 모든게 참이어야 참. EXISTS: ?
...
-- SELECT 2 : Whalen 보다 월급을 많이 받는 사원 조회 --> 이렇게 Whalen의 월급이 얼만지 알아낸 다음에 아래 명령문 수행해서 보다 큰 월급 받는 직원 뽑아내는 과정.
SELECT last_name, salary
FROM employees
WHERE salary >= 4400; --Whalen의 월급지정.. --> 이걸 간편하게 서브쿼리로 가능 !!
...
SELECT -- 메인쿼리
    ...
    (   -- 상관부속질의: 메인쿼리의 결과값을 사용해야만 수행가능한 쿼리
        SELECT t.first_name  --> 얘는 상관 부속질의. 메인쿼리가 주는 값을 가지고 수행되어야 하기 때문에.
        FROM employees t
        WHERE t.employee_id = m.employee_id --< 메인쿼리가 주는 값. --> 서브쿼리의 사원번호인지, 메인쿼리의 사원번호인지 오라클이 알 수 없음. 별칭사용.
    ) AS first_name,
    last_name,
    salary
FROM
    ...
    -- 비상관 부속질의
    (SELECT * FROM employees WHERE last_name != 'Whalen') m --> 여기서 웰런의 이름을 빼고 하나의 테이블이 WHERE 절로 들어감. 이렇게 필요에 따라 모든 절에 사용가능.
WHERE
    -- 2. Whalen 보다 많은 월급여를 받는 사원조회
    salary >= (
        -- 서브쿼리 (=비상관 부속질의)
        -- 단독 수행에 지장없음 -----> 메인쿼리가 없더라도 진행될 수 있는 쿼리라서 비상관 부속질의 !
