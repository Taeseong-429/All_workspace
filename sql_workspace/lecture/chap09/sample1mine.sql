...
-- Basic syntax:
--
--  CREATE [OR REPLACE] VIEW 뷰이름 [ (alias1, alias2, ...) ] --> REPLACE는 생성하거나 또는 대체하라는 의미.
--                                                                지정한 뷰가 지금 새로만드는거면 만들고, 있으면 대체하라는 의미.
--  AS
--  Sub-query
...
CREATE OR REPLACE VIEW emp_view3 (사원번호, 이름, 월급)
AS
...
-- ------------------------------------------------------
-- (*주의*) View 생성시, 함수를 사용하는 경우에는 **반드시****** 반드시 !
...
--        경우에는, DML 변경 불가!!!! (책) --> 그룹핑하고 집계함수 등 더이상 개별 레코드를 보지 않겠다는 얘기니까.
...
SELECT 
    DISTINCT        -- DISTINCT 키워드 사용!!!
        empno, ename, sal, deptno --> DISTINCT 이지만, 아예 일치하는 행이 없었기 때문에 중복행으로 처리 되지 않아서 삭제 안됨.
                                  --> 그래서 여긴 책의 내용과 다르게 잘 된거임.
FROM emp;
...
FROM
    employees t1,
    departments t2 --> 동등조인으로 만든 복합 View. 절대 DML문장 안됨 ! --> Developer에서 해야함 !
WHERE
...
FROM emp
WHERE deptno = 30
WITH CHECK OPTION; --> 30번 부서의 직원들만 데이터 변경 가능하게 만드는게 WITH CHECKED OPTION 인듯.
...
UPDATE emp_view6
SET 
    deptno = 40,     -- XX 이거만 주석체크하면 아래 조건은 이동 됨.
    sal = 10000         -- OK
WHERE empno = 7521;


-- ------------------------------------------------------
-- (4) WITH READ ONLY 제약조건 -- > 읽기 전용 모드로 만들기.
-- ------------------------------------------------------
